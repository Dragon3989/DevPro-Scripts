...
function c51091138.filter(c,seq,tp)
...
function c51091138.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_ONFIELD) and c51091138.filter(chkc,e:GetLabel(),tp) end
	local c=e:GetHandler()
	local seq=c:GetSequence()
	if chk==0 then return Duel.IsExistingTarget(c51091138.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,c,seq,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c51091138.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,c,seq,tp)
	e:SetLabel(seq)
